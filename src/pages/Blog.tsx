    ...
    <main className="min-h-screen bg-black pt-24 pb-20">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {sortedArticles.length > 0 && (
          <Link to={`/blog/${sortedArticles[0].slug}`} className="group block mb-16">
            <motion.div
            ...
            >
              <div className="aspect-[21/9] relative">
                <ImageWithSkeleton
                  src={sortedArticles[0].heroImage}
                  alt={`${sortedArticles[0].title} — ${sortedArticles[0].category} article on CinemaDiscovery`}
                  fetchPriority="high"
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12">
                  ...
                  <div className="flex items-center gap-4 text-gray-400 text-xs font-sans">
                    <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" /> {new Date(sortedArticles[0].publishDate || sortedArticles[0].date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                    <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> {sortedArticles[0].readTime}</span>
                  </div>
                </div>
              </div>
            </motion.div>
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 text-gray-500 text-xs font-sans">
                      <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {new Date(article.publishDate || article.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
