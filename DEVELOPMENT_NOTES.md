...
### July 21, 2026
- **Blog Article #20 Published** — Added "Quentin Tarantino Movies Ranked | Worst to Best" to `src/data/blogArticles.ts`. Slug: `quentin-tarantino-movies-ranked`. Ranks all 10 Quentin Tarantino films from worst (Death Proof) to best (Pulp Fiction). SEO meta tags, canonical URL (`https://cinemadiscovery.com/blog/quentin-tarantino-movies-ranked`), and JSON-LD `BlogPosting` schema included. Verified TMDB movie IDs via API: Pulp Fiction (680), Kill Bill: Vol. 1 (24), Kill Bill: Vol. 2 (393), Reservoir Dogs (500), Inglourious Basterds (16869), Django Unchained (68718), Once Upon a Time in Hollywood (466272), Jackie Brown (184 — corrected from candidate 74), Death Proof (1991 — corrected from candidate 1885), and The Hateful Eight (273248). Natural cross-links to `/blog/top-10-directors-of-all-time`, `/blog/best-thriller-movies-of-all-time`, `/blog/movies-with-best-cinematography`, and CTA to `/movies`. Updated `scripts/generate-sitemap.js` so sitemap XML outputs `<lastmod>2026-07-21</lastmod>`. Build passed zero errors (2372 modules transformed, 439 static HTML shells pre-rendered).

### July 9, 2026
- **Mood Engine — Load More Pagination** — Replaced the "Want Different Picks?" button with a "Load More" button that appends 20 films per click (grid grows: 20 → 40 → 60…). Rewrote `src/hooks/useMoodDiscovery.ts` as a stateful accumulator hook: page managed internally, ref-backed `Set<id>` deduplicates across TMDB pages, exposes `films[]`, `loadMore()`, `reset()`, `hasMore`, `isFetching`, `isError`, `refetch()`. Updated `src/components/discover/MoodResults.tsx`: removed `onFreshPicks` prop and "Want Different Picks?" button; added "Load More" (crimson primary, spinner-on-fetch, disabled-during-fetch) below the results grid; shows "That's all we found" in muted text when `hasMore=false`; header count reflects live `films.length`. Updated `src/pages/Discover.tsx`: removed `page` state and `handleFreshPicks` callback; `handleRestart` calls `reset()` which clears accumulated films, sets page back to 1, and resets `hasMore` to true. 100-vote floor, rating sort, genre/keyword/exclusion locks, 6s timeout, and `keepPreviousData` all preserved. Build passed zero TypeScript errors (2372 modules). Commit: `62d715d`. Only 3 files changed (strict lock honoured).
...
### June 13, 2026
- **Blog Article #16 Published** — Added "Best Heist Movies of All Time" as the new top article in `src/data/blogArticles.ts`. Slug: `best-heist-movies-of-all-time`. Covers Heat, Ocean's Eleven, Rififi, Dog Day Afternoon, The Usual Suspects, Reservoir Dogs, Inside Man, The Town, Baby Driver, and Den of Thieves. OG/hero image: `https://image.tmdb.org/t/p/w1280/xKsnZDERG1dk95wuZ5q9iks3OL3.jpg` (Heat 1995 TMDB backdrop, ID 949, verified via API). Internal links added for all 10 confirmed TMDB IDs: Heat (949), Ocean's Eleven (161), Rififi (934), Dog Day Afternoon (968), The Usual Suspects (629), Reservoir Dogs (500), Inside Man (388), The Town (23168), Baby Driver (339403), Den of Thieves (449443) — all verified via TMDB API before insertion. Sitemap updated (439 URLs). Prerendered HTML shell at `dist/blog/best-heist-movies-of-all-time/index.html` confirmed with correct OG tags, Twitter Card, and BlogPosting JSON-LD schema (author: Ahmed Khan, datePublished: 2026-06-13, mainEntityOfPage: canonical URL). Note: `Home.tsx` had pre-existing JSX syntax errors from a prior uncommitted edit; these are not related to this article change — the build passes cleanly when only `blogArticles.ts` is changed, as confirmed by git stash test (2363 modules, zero errors).
